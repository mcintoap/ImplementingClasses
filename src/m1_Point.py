...
class Point(object):
    ...
    def  move_to(self, x, y):
        self.total_distance = self.total_distance + ((x-self.x)**2 + (
            y-self.y)**2)**.5
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.moves_made = self.moves_made +1
        self.total_distance = self.total_distance + (dx**2 + dy**2)**.5
    ...
    def get_distance_from_start(self):
        return((self.initial_x-self.x)**2 + (self.initial_y-self.y)**2)**.5

    def get_distance_traveled(self):
        return self.total_distance

    def closer_to(self,p2,p3):
        if ((((self.x-p2.x)**2 + (self.y-p2.y)**2)**.5)  > ((self.x-p3.x)**2
                                                               + (
                        self.y-p3.y)**2)**.5):
            return p3
        elif ((((self.x-p2.x)**2 + (self.y-p2.y)**2)**.5)  == ((
                                                             self.x-p3.x)**2
                                                               + (
                        self.y-p3.y)**2)**.5):
            return p2
